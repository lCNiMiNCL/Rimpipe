...
- **阶段 4（健壮性与性能）进行中：** 已实现旧档容器按 Props 补齐（additive，不升 schema）；已实现破损标志局部化与 `NotifyBreachChanged(Thing)` 精确唤醒，并已游戏内确认通过；已实现泵/阀/换热器/反应釜下标保护统一，并已游戏内确认通过；已实现拓扑重建分配优化（scratch 复用，待游戏验证）。
