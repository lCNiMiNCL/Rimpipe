...
> **状态：** 阶段四 **4.1–4.8 ✅**；**4.9 延后**；**4.10 DirtyTopo 本体验收通过**（§7.10.12 · `0.4.7`）；**4.11 发布 R1 ✅**（§1.2s · `0.4.0`）；**4.12 Debug 整理 ✅**（§1.2t · `0.4.1`）；**Overlay 性能修复验收通过**（§1.2u / §7.3.f · `0.4.2`）；**批级优化验收通过**（§7.10.8 · `0.4.4`）；**休眠重评估单遍聚合验收通过**（§7.10.9 · `0.4.5`）；**代码审查 8 项修复验收通过**（§7.10.10 · `0.4.5`）；**批级缓存验收通过**（§7.10.11 · `0.4.6` · 暂不 push）；**6.18 流体物理量 FluidPhysics ✅ / 6.19 管道连接分组 PipeChannels ✅**（§6.18 / §6.19 · R-通道 4/4 · 2026-08-10）  
...
### 6.18 决议：流体物理量 — FluidPhysics（粘度 / 比热 · 2026-08-10 已锁定）

> **玩家确认：** 粘度→流动阻力，比热→传热；泵也吃粘度；Amb 驱动上限也换热容。  
> **问题背景：** FluidDef 原只有泄漏效果字段；水/燃料/液氧在流动与传热上表现完全一致（「不同流体物理量没有区别」）。

#### 6.18.1 架构边界

| 项 | 决议 |
|----|------|
| 作用层 | 求解期动态读流体属性；**不改** Pr-A / R-A / 泄漏语义 / 批时序 |
| 粘度 | `FluidDef.viscosity`（默认 1）；批内 `rateCap = maxFlowRate / viscosity`（Equalize **与 Forced 泵都吃**）；泄漏扣量=大气孔**不吃** |
| 粘度取端 | 同边两端流体必相同（异流体 want=0），任取 A 端即可 |
| 比热 | `FluidDef.specificHeat`（默认 1）；`Q/m → Q/(m·c)` 三处必改：导热温变 / Amb（驱动 `\|ΔT\|/2·m·c` + 响应 `Q/(m·c)`）/ 反应热；**混温数学免改**（同流体 c 约掉） |
| 校验 | ConfigErrors：viscosity/specificHeat > 0（防除零） |
| 存档 | 属性在 Def 数据，Container 不加字段；**schema 仍 1** |
| 兼容 | v=c=1 时全部既有公式逐位退化；既有 5 套件断言数值不变 |

#### 6.18.2 任务清单

- [x] 6.18.a FluidDef +viscosity/+specificHeat + ConfigErrors
- [x] 6.18.b FlowSolver：Equalize/Forced 的 rateCap 除粘度
- [x] 6.18.c HeatSolver：导热驱动换 `min(m·c)`；+`SpecificHeatOf`
- [x] 6.18.d MapComp：AccumulateHeat / ApplyAmbientHeatExchange / ApplyChemReactionHeat 三处 Q/(m·c)
- [x] 6.18.e Defs：4 流体显式 v=c=1；新增 `TestThick`(v=2) / `TestHeatSink`(c=2) 验收流体 + DefInjected
- [x] 6.18.f R-通道 粘度/比热断言（§6.19 套件内）

#### 6.18.3 验收记录（Player.log · 2026-08-10）

| 项 | 结论 | 证据 |
|----|------|------|
| 粘度 | ✅ | `粘度通过：基准(v=1) 右增 48.42 > 稠液(v=2) 右增 27` |
| 比热 | ✅ | `比热通过：水腔(c=1) 温变 3.8 ≈ 2× 高比热腔(c=2) 温变 1.7` |
| 回归 | ✅ | 既有 5 套件满分（R-框架 8/8 · R-物理 7/7 · R-热与环境 6/6 · R-化学 5/5 · R-扩展 2/2） |

> **断言踩坑（记录）：** 粘度效果=收敛速度（单批 transfer 少）。跑 6/2 批后两种流体都已收敛到均衡（50 vs 49.54）断言失效；正确窗口是 **1 批**（水≈43 稠≈27，差≈16）。

---

### 6.19 决议：管道连接分组 — PipeChannels（A/B 双通道隔离 · 2026-08-10 已锁定）

> **玩家确认（多轮澄清）：** 管道不再 4 邻自动全通；4 个方向出口各属 A/B/不通（AB 不能同占一个方向）；**AB 是无顺序的两个量**。  
> **模型（玩家原话）：** 「一个格子的 A 可以连到邻居的 B 上」——管道之间**不看组**；「A 组是东西 B 组是南北就相当于互不相连的十字交叉」——格内同组互连、跨组隔离。  
> **叙事：** AB = 不同高度的管道层；普通设备只够得到 A 层（端口默认 A），B 层太低需 B 端口设备（`channel=1`）才能接。

#### 6.19.1 架构边界

| 项 | 决议 |
|----|------|
| 数据 | `CompPipeCell.groupAMask`（bit=Rot4.AsInt，默认 **0b1111** 全通）/ `groupBMask`（默认 **0**）；AB 不同占（SetDir 自动互斥） |
| 格内 | 同组出口互连、跨组隔离（A={E,W} B={N,S} = 互不相连十字；A={N,E} B={S,W} = 两个 L） |
| 邻居 | 双方对向出口都开就连通，**不看组**（A 出口可直接连 B 出口）；组只决定格内拐弯方向 |
| 设备 | `Port.channel`（0=A 默认 / 1=B）；接入须管道格该方向出口组 == 端口组，否则悬空 |
| 拓扑 | 洪水 / Voronoi / 最短路 / DirtyTopo / 环路重连全链路升级为「节点（格子×通道）」空间；`NotifyPipeConnectionChanged` 复用 `PipeChanged` 局部重建 |
| Gizmo | 组选择（A/B）+ 4 方向逐向开关；Inspect 显示两组方向 |
| 存档 | 新增带默认值字段；**schema 仍 1**；旧档读入 = A 全通 = 旧行为 |
| 兼容 | 新建管道 A 全通 B 空 = 旧行为；既有 5 套件与全部 Debug 场景零改动 |

#### 6.19.2 任务清单

- [x] 6.19.a CompPipeCell：组掩码 + DirGroup/IsOpenWorld/SetDir + Scribe + Gizmo + Inspect
- [x] 6.19.b Port / PipePortProp +channel + Scribe + ConfigErrors；EnsureRuntimeObjects 复制
- [x] 6.19.c MapComp 拓扑节点化（FloodPipeComponent / FloodComponentSet / FloodComponentFromAllChannels / BuildPipeAdjacentMappings / BuildPipeComponent / ShortestPipePathCellCount / Collect* / RebuildDirtyLocal / NotifyPipeConnectionChanged）
- [x] 6.19.d Keyed 简中/英 + DefInjected
- [x] 6.19.e R-通道套件（十字隔离 / 断开恢复 / 粘度 / 比热 4 断言）+ TestThick/TestHeatSink 流体

#### 6.19.3 验收记录（Player.log · 2026-08-10）

| 项 | 结论 | 证据 |
|----|------|------|
| 十字隔离 | ✅ | `双通道十字通过：EW=True NS=True 跨线=False 东100→50 西0→50 北60→60 南60→60` |
| 断开/恢复 | ✅ | `方向断开恢复通过：前=True 断=False 恢复=True` |
| 粘度 / 比热 | ✅ | §6.18.3 |
| R-通道 | ✅ | `R-通道：通过 4/4` |
| 回归 | ✅ | 既有 5 套件满分 |
...
- **B 端口产品设备**：现有储罐/三通/阀/泵/换热器全为 A 端口；接入 B 组管网需 Def 配 `<channel>1</channel>`（代码链路已就绪，玩法设备待做）。
- **性能**：默认（B 空）节点数=格数，拓扑重建成本≈旧版；B 组实际使用时节点数最多 ×2（偶发重建 ≤2×）；批级热点（Acc/Commit）零变化（Mapping 未加字段）。
...
> **范围：** 仅 [MapComponent_PipeNetwork.cs](../../RimPipe/Map/MapComponent_PipeNetwork.cs) 批处理实现层；**不改** Flow/Heat 公式、拓扑、存档（schema 仍 1）、DefName。
...
> **范围：** 仅 [MapComponent_PipeNetwork.cs](../../RimPipe/Map/MapComponent_PipeNetwork.cs) 实现层 + 2 个 Debug 断言键；**不改** Flow/Heat 公式、Voronoi/直接相邻/内部映射算法语义、休眠判定规则、存档（schema 仍 1）、DefName。
